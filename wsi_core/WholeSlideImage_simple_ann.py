...
#            print("hole:",hole)
    ...
    @staticmethod
    def isInRoI(pt, patch_size, ann_path):
        '''
        フォルダでも読み込む場合：
        ⓵ ann_pathを、isFileでファイルかフォルダかを判定
        ⓶ ファイルの場合は以下の処理、フォルダの場合はglobでファイル名を取り出し
        ⓷ 
...
#            print("roi:",roi)
    ...
    # pt=パッチのstart座標[x,y]
    @staticmethod
    def isInContours(cont_check_fn, pt, holes=None, patch_size=256, ann_path=None): # パス追加
#        print("ann_path:",ann_path)
        ...
        if cont_check_fn(pt):
            ...
            if holes is not None:
                # パッチの中心点が穴に入っていなかくてroiに入ってたらTrue、roiに入ってなかったらFalse、穴に入っていたらFalseを返す
#                return not WholeSlideImage.isInHoles(holes, pt, patch_size)
                if not WholeSlideImage.isInHoles(holes, pt, patch_size):
                    # ann_pathがあればroiに入ってたらTrue,入ってなかったらFalse
                    if ann_path is not None:
                        return WholeSlideImage.isInRoI(pt, patch_size, ann_path)
                    # ann_pathがなければTrue
                    else:
                        return 1
                else:
...
#        print("ann_path:",ann_path)
        ...
        iterable = [(coord, contour_holes, ref_patch_size[0], cont_check_fn, ann_path) for coord in coord_candidates]
        ...
        for i in iterable:
            ...
            iterable_line = [np.uint32(i[0][0]).item(), np.uint32(i[0][1]).item()], i[1], i[2], i[3], i[4]
...
#        results = pool.starmap(WholeSlideImage.process_coord_candidate, iterable)
    ...
    @staticmethod
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn, ann_path=None):
#        print("ann_path_proc:",ann_path)

        # パッチが組織の輪郭内に入っており、中心点が穴に入っていない場合は、パッチのx,y座標を返す
        if WholeSlideImage.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size, ann_path):
            return coord
        else:
            return None
