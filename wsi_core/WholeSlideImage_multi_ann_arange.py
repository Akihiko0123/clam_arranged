...
#        print("slide_name:",slide_name)
        ...
        if os.path.isfile(ann_path):
            ...
            rois = []
            # qupathの仕様に合わせて、アノテーション座標を1つずつ取り出し、アノテーション内にパッチ中心部があるかどうかチェック
            for fd in d['features']:
                # アノテーション座標のリストの形状の次元数が3でない([1,150,2]等のはずが[2,]等の場合)、
                # 又は、形状の最初の値が1でない([2,2,2]等)場合は、fd内にアノテーションが二つ以上入っていると判断する。
                if len(np.array(fd['geometry']['coordinates']).shape) != 3 or np.array(fd['geometry']['coordinates']).shape[0] != 1:
                    ro = fd['geometry']['coordinates']
                    # 複数のアノテーションが入ったリストから、アノテーションを1つずつ取り出して、roisリストに入れる。
                    for r in ro:
                        rois.append(r)
                else:
                    # アノテーションをroisに入れる。
