...
# WholeSlideImage.pyのprocess_contoursメソッドから呼び出される。
# 保存先パス、組織パッチ情報、スライド情報(スライド名、パッチレベル等)を元に処理される。
def save_hdf5(output_path, asset_dict, attr_dict= None, mode='a'):
    file = h5py.File(output_path, mode)
    # key＝"coords",val=パッチstart x,y座標リスト(Tumor.ndpiの場合、組織の輪郭には1738個の座標(パッチ)データが入っている)
    for key, val in asset_dict.items():
        print("key:",key)
        print("file:",file)
        data_shape = val.shape # 例: (1738,2) 2はx,y
        print("val.shape:",data_shape)
        # fileにkey('coords')が含まれない場合、
        if key not in file:
            print("keyはfileに含まれない!")
            data_type = val.dtype # intと思われる
            chunk_shape = (1, ) + data_shape[1:] # (1,2)
            maxshape = (None, ) + data_shape[1:] # (None,2)
            # データセットを作成する。
            # 引数(データセット名='coords',shape=(1738,2), maxshape=(None,2),chunks=(1,2),dtype=int32?)
            # chunksはデータセットの分割の仕方を表す。maxshapeは設定することで、chunksが設定されてなくても自動チャンキングが行われるとのこと。
            dset = file.create_dataset(key, shape=data_shape, maxshape=maxshape, chunks=chunk_shape, dtype=data_type)
            # データセットにパッチのstart座標値のリストを入れる。
            dset[:] = val
            # attr_dict(ファイル特性情報)がある場合(default＝あり)、
            if attr_dict is not None:
                # attr_dict.keys()=dict_keys(['coords'])の中にはkey='coords'が入っているので、条件に合う
                if key in attr_dict.keys():
                    # データセットdset.attrs内に、attr_keyとattr_valのセットを入れていく
                    # attr_key＝パッチサイズ,パッチレベル,ダウンサンプル,tuple(np.array(パッチレベルでの画像サイズ)),
                    #           パッチレベルでの画像サイズ,ファイル名,保管先パス
